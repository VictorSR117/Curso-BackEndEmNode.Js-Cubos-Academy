const checkAuthorizationPromotion = (dateHomePromotion, dateRequestCustomer) => {
    ...
    if (dateRequestCustomer >= dateHomePromotion) {
        ...
        if (differenceInDays <= 24) return true; // O cliente está autorizado a usufruir da promoção
    }
    return false; // A promoção não está autorizada
}

// Exemplo de uso da função
const dateHomePromotion = new Date(2023, 8, 1, 12); // Data de início da promoção
const dateRequestCustomer = new Date(2023, 8, 25, 12); // Data em que o cliente solicitou a promoção
